...
class LocalConnector(PlannerConnector):
    ...
    async def add_component(self, component_name: str, blocking: bool = True) -> bool:
        ...
        if not ray.is_initialized():
            # 尝试自动连接到已有的 Ray 集群
            # 如果你是在 multi‑node 或者 Kubernetes 上跑的，通常用 address="auto"
            ray.init(address="auto", ignore_reinit_error=True)
        print(f"[AKing info] LocalConnect ray._private.state.actors()={ray._private.state.actors()}")
        state = await self._load_state()
        # Find max suffix
        max_suffix = 0
        ...
        print(f"[AKing info] component_name:{component_name}\t service_config: {service_config}")
        # Build environment
