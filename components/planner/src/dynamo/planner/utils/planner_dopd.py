...
class Planner:
    ...
    def calculate_optimal_pd(self, isl: float, osl: float):
        """
        ...
        block_size = 128
        num_isl_blocks = math.ceil(isl/block_size) 
        num_osl_blocks = math.ceil(osl/block_size) 
        max_decode_concurrency = num_available_kv_blocks // (isl+(ols/2))
        next_ttft = self.prefill_interpolator.interpolate_ttft(isl)
        next_tpot = self.decode_interpolator.interpolate_itl(max_decode_concurrency, isl+(ols/2))
        pd_rate_optimal = max_decode_concurrency / ((osl * next_tpot) / next_ttft)
        return pd_rate_optimal
