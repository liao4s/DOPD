...
class VllmV0ConfigModifier:
    ...
    @classmethod
    def get_model_path(cls, config: dict) -> str:
        if "Common" in config and "model" in config["Common"]:
            return config["Common"]["model"]
        else:
            return config[WORKER_COMPONENT_NAMES["vllm_v0"].decode_worker]["model"]
