...
class FrontendConfig(BaseModel):
    ...
    endpoint_chat: str = None
    endpoint_completion: str = None
    port: int = 8080
...
class Frontend:
    ...
    def setup_model(self):
        ...
        if self.frontend_config.endpoint_chat:
            subprocess.run(
                [
                    "llmctl",
                    "http",
        ...
        if self.frontend_config.endpoint_completion:
            subprocess.run(
                [
                    "llmctl",
                    "http",
            ...
            subprocess.run(
                [
                    ...
                    "completions",
                    self.frontend_config.served_model_name,
                    self.frontend_config.endpoint_completion,
                ],
                check=False,
            )
